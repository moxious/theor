...
public class Interval {
	...
	public String getName() {
		return Interval.name(this);
	}

	public boolean isWholeStep() {
		return Math.abs(i) == WHOLE_STEP.intValue();
	}

	public boolean isHalfStep() {
		return Math.abs(i) == HALF_STEP.intValue();
	}
	...
	public static Interval[] parse(String str) {
		...
		for (int x = 0; x < parts.length; x++) {
			...
			if (map.containsKey(s))
				...
				System.err.println("Interval#parse: Unrecognized interval '" + s + "'");
		}
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(
				"A 3rd from " + c + " is " + THIRD.apply(c) + " and its inversion is " + THIRD.inverse().apply(c));
	}
	...
	public static String name(Interval[] ivs) {
		...
		if (ivs == MAJOR_TRIAD)
			return "major triad";
		if (ivs == AUGMENTED_TRIAD)
			return "augmented triad";
		if (ivs == DIMINISHED_TRIAD)
			return "diminished triad";
		if (ivs == DIMINISHED_SEVENTH_TETRAD)
			return "diminished seventh tetrad";
		if (ivs == HALF_DIMINISHED_TETRAD)
			return "half diminished tetrad";
		if (ivs == MINOR_SEVENTH_TETRAD)
			return "minor seventh tetrad";
		if (ivs == MAJOR_SEVENTH_TETRAD)
			return "major seventh tetrad";
		if (ivs == AUGMENTED_SEVENTH_TETRAD)
			return "augmented seventh tetrad";
		if (ivs == AUGMENTED_MAJOR_SEVENTH_TETRAD)
			return "augmented major seventh tetrad";
	...
	private static void initializeMap() {
		...
		map.put("w", WHOLE_STEP);
		map.put("h", HALF_STEP);
		map.put("m3", MINOR_THIRD);
		map.put("3", THIRD);
		map.put("4", FOURTH);
		map.put("5", FIFTH);
		map.put("6", SIXTH);
		map.put("7", SEVENTH);
		map.put("8", OCTAVE);
		map.put("b3", MINOR_THIRD);
		map.put("b2", MINOR_SECOND);
		map.put("b5", DIMINISHED_FIFTH);
		map.put("b6", MINOR_SIXTH);
		map.put("#7", AUGMENTED_SEVENTH);
		map.put("#4", TRITONE);
		map.put("1", UNISON);
		map.put("#5", AUGMENTED_FIFTH);
		map.put("b7", MINOR_SEVENTH);
