...
public class MelodyMaker {
	...
	public Note chooseNote(float duration) {
		...
		log.fine("Choose note: " + tn + " OCTAVE " + chosenOctave + " MIDI " + tn.getMIDI() + " from last note "
				+ getLastNote());
	...
	public Pattern makePattern(byte instrument) throws TheorException {
		...
		while (timePos < endTime) { // Still need to add more notes. 			
			...
			log.fine("\nMelodyMaker loop: so far " + beatsAdded + " of " + beats + " time sequence " + timePos
					+ " swing " + swing + " present " + (present == null ? "N/A" : present.getName()) + " future "
					+ (coming == null ? "N/A" : coming.getName()) + " at " + whenChange + " AFFINITY "
					+ affinity.getName());
			...
			if (ProgressionComposer.r.nextDouble() <= pauseProbability && (duration * 2) <= whenChange)
				duration *= 2;
	...
	public double getPctStageRemaining(double timePos, double lastChange, double whenNextChange) throws TheorException {
		if (whenNextChange == -1)
			return 0.0;
		if (whenNextChange == lastChange)
			return 1.0;

		if (timePos > whenNextChange)
			throw new TheorException("Timepos " + timePos + " cannot be greater than next change " + whenNextChange);
		if (timePos < lastChange)
			throw new TheorException("Timepos " + timePos + " cannot be less than previous change " + lastChange);
		if (whenNextChange < lastChange)
			throw new TheorException(
					"Next change " + whenNextChange + " cannot be smaller than previous change " + lastChange);
	 ...
	 */
	public Chord chooseChordAffinity(double timePos, double lastChange, double whenNextChange, Chord present,
			Chord coming) throws TheorException {
		if (present != null && coming == null)
			return present; // only one choice.
		else if (present == null && coming != null)
			return coming; // only one choice.
		else if (lastChange == whenNextChange)
			return present;
		else {
			if (timePos > whenNextChange)
				throw new TheorException(
						"Timepos " + timePos + " cannot be greater than next change " + whenNextChange);
			if (timePos < lastChange)
				throw new TheorException("Timepos " + timePos + " cannot be less than previous change " + lastChange);
			if (whenNextChange < lastChange)
				throw new TheorException(
			...
			if (randVal > pctRemaining)
				return coming; // Increasingly unlikely as pctDone gets bigger, but likely early on.
	...
	public int pickOctave(Note lastNote, int chosenToneClass) {
		...
		int chosen = -1;
		if (dist1 < 6)
			chosen = lastNote.getOctave();
		if (dist2 < 6)
			chosen = lastNote.getOctave() + 1;
		if (dist3 < 6)
			chosen = lastNote.getOctave() - 1;

		if (chosen < (DEFAULT_OCTAVE - 1))
			chosen = DEFAULT_OCTAVE - 1;
		if (chosen > (DEFAULT_OCTAVE + 1))
			chosen = DEFAULT_OCTAVE + 1;
