...
public class ToneNet {
	...
	public List<Double> getWeights() {
		...
		for (double we : weights)
			w.add(we);
		return w;
	}

	public void clear() {
		weights = new double[] { BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT,
				BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT, BASE_WEIGHT };
	} // End clear
	...
	public void bias(Chord c, Note last, double resolutionBias) throws TheorException {
		...
		NSequence toDiscourage = new NSequence(new Note[] { Interval.TRITONE.apply(chordTonic),
				Interval.AUGMENTED_FIFTH.apply(chordTonic), Interval.TRITONE.apply(tonic),
				Interval.AUGMENTED_FIFTH.apply(tonic), Interval.MINOR_SECOND.apply(tonic),
				Interval.SEVENTH.apply(tonic), Interval.MINOR_SECOND.apply(chordTonic) });

		for (Note n : toDiscourage)
			influence(n, BIG_DISCOURAGE);
		...
		for (int x = 0; x < 12; x++) {
			if (homeSequence.contains(x))
				influence(x, LITTLE_ENCOURAGE);
			else
				influence(x, VETO);
		}
		...
		if (last != null)
			influence(last, LITTLE_DISCOURAGE);
