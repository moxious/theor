 ...
 */
public class MLoad {
	public static Map<Integer, NoteCollection> midi2NSequence(File midi)
			throws IOException, InvalidMidiDataException, TheorException {
		Player p = new Player();
		...
		for (String tok : toks) {
			...
			if (mmt.find()) {
				...
				if (moments.containsKey(activeMoment))
					active = moments.get(activeMoment);
				else {
			...
			if (active != null && active.countNotes() >= 3 && active instanceof NSequence
					&& ((NSequence) active).getPitchClasses().size() >= 3) {
	...
	protected static Integer parse(String noteName) throws TheorException {
		...
		case 'g':
			...
			break;
		default:
			throw new TheorException("Invalid note name '" + noteName + "'");
		}
		...
		for (Integer k : list) {
			if (moments.get(k).countNotes() > 0) {
				if (moments.get(k) instanceof Chord) {
					System.out.println("MOMENT " + k + " + is a chord: " + moments.get(k) + " / "
							+ ((Chord) moments.get(k)).getName());
				} else
					System.out.println(
							"MOMENT " + k + " has " + moments.get(k).countNotes() + " notes: " + moments.get(k));
				if (moments.get(k).countNotes() > max)
					max = moments.get(k).countNotes();
			}
		}
