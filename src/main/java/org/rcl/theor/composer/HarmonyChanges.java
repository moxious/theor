...
public class HarmonyChanges {
	...
	public void addChange(Chord c, double duration) throws TheorException {		
		if(duration <= 0) throw new TheorException("No negative or zero durations permitted"); 
	...
	public double whenEnds(int chord) throws TheorException { 
		if(chord < 0 || chord >= chords.size()) throw new TheorException(chord + " out of range"); 
		
		double tot = 0.0;
		for(int x=0; x<=chord; x++) tot += durations.get(x); 
	...
	protected String qlist() { 
		...
		for(double d : durations) b.append(""+d + " "); 
		return b.toString();
	}
	...
	public double getComingChordTiming(double timePos) { 
		int idx = getIndex(timePos);
		if(idx == durations.size()-1) return -1;
	...
	public Chord getComingChord(double timePos) { 
		int idx = getIndex(timePos);
		if(idx >= chords.size()-1) return null; // Nothing's coming, you're out of road.		
		return chords.get(idx+1);
	}
	
	protected int getIndex(double timePos) { 
		if(timePos < 0) return 0;
		
		double tot = 0; 
		
		for(int x=0; x<durations.size(); x++) { 
			if(timePos <= (durations.get(x) + tot) && timePos >= tot) return x;
