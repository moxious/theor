...
public class ProgressionComposer extends Composer {
	...
	protected void initializeProgression(DataFeed df) throws TheorException {
		...
		log.fine("With a tempo of " + tempo + " and a constraint of " + TIMEFRAME_SECS + " sec, we could get "
				+ maxBeats + " beats.");
	...
	public String getName() {
		...
		return "Len-" + bytes + "_" + center + "_PROG_" + cProg.getFormula() + "_harmony_"
				+ Instrument.INSTRUMENT_NAME[harmonyInstrument] + "_melody_"
				+ Instrument.INSTRUMENT_NAME[melodyInstrument] + "_tempo_" + tempo + "_mOctave_" + melodyOctave
				+ "_hOctave_" + harmonyOctave + "_scale_" + scale.name;
	}
	...
	public Pattern makeHarmony() throws TheorException {
		...
		for (int x = 0; x < chordCount; x++) {
			durations[x] = Syncopation.QUARTER_NOTE;
		}
	...
	public static void main(String[] args) throws Exception {
		String[] strings = new String[] { "David Rafner", "Hedge Fund", "a", "mdavidallen", "th3j35t3r", "pca",
				"testing", "twitter", "FACEBOOK", "just_for_yuks", "meatbus", "12132143213" };
		...
		for (String s : strings) {
			...
			// p.play(pat);
			p.saveMidi(pat, new File(s + "-" + cmp.getName() + ".mid"));
